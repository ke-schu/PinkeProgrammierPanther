...
{
    /**
     * Der Konstruktor erstellt ein Ereignis vom Typ Heiler. Heiler sind Ereignisse, die es dem Spieler ermoeglichen
     * seine Lebenspunkte zu regenerieren, wenn er vorher Schaden genommen hat.
     *
    ...
    /**
     * Diese Methode ueberlagert die Methode aus der Superklasse "Ereignis". Der Heiler prueft ob die Interaktion eine
     * Bezahlung erfordert. Je nach Resultat werden die Lebenspunkte des Spielers kostenlos oder gegen eine Bezahlung
     * regeneriert.
     *
    ...
    public void ausfuehren (SpielStand spiel)
    {
        if (isAuswahl())
        {
            ...
            {
                if (spiel.getSpieler().getLebenspunkte() + HEILER_AUFWERTUNG_LEBENSPUNKTE <
                    spiel.getSpieler().getMaxLeben())
                {
                    spiel.getSpieler()
                         .setLebenspunkte(spiel.getSpieler().getLebenspunkte() + HEILER_AUFWERTUNG_LEBENSPUNKTE);
                    gratisInteraktionen--;
                }
                else if (spiel.getGold() >= this.getKosten())
                {
                    ...
                    if (spiel.getSpieler().getLebenspunkte() + HEILER_AUFWERTUNG_LEBENSPUNKTE < spiel.getSpieler().getMaxLeben())
                    {
                        spiel.getSpieler()
                             .setLebenspunkte(spiel.getSpieler().getLebenspunkte() + HEILER_AUFWERTUNG_LEBENSPUNKTE);
                    }
