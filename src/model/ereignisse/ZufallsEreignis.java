...
/**
 ...
 Wahrscheinlichkeit. Ein ZufallsEreignis ist fuer den Spieler erst erkennbar,
 wenn dieser es ausfuehrt. */
public class ZufallsEreignis extends Ereignis implements Wahrscheinlichkeit
{
    ...
    /**
     ...
     ZufallsEreignis ist ein Ereignis, dessen genaue Art fuer den Spieler erst
     bekannt wird, wenn er den zugehoerigen Raum betritt.
    ...
    public void ausfuehren (SpielStand spiel)
    {
        if (!ausgefuehrt)
        {
            if (isAuswahl())
            {
                ...
                {
                    spiel.getSpieler().setMacht(
                            spiel.getSpieler().getMacht() +
                            ZE_MACHT_ERHOEHUNG);
                    ereignisnummer = ZE_2;
                }
                else if (wahrscheinlichkeit <= VIERZIG_PROZENT)
                {
                    spiel.getSpieler().setMana(
                            spiel.getSpieler().getMana() + ZE_MANA_ERHOEHUNG);
                    ereignisnummer = ZE_3;
                }
    ...
    /**
     ...
     "wahrscheinlichkeit",
     indem ein zufaelliger Wert zwischen 0.0 und dem maximalen Wert der
     Skalierung gebildet wird.
